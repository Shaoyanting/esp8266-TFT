...
#include <DHT.h>
#define DHTPIN 5  //设置用于连接温湿度传感器的引脚
...
//MQTT通讯相关
String controlTopic = "ZUCC-SYT/control"; // 设备控制主题
String chatTopic = "ZUCC-SYT/chat";       // 用户消息主题
String tipTopic = "ZUCC-SYT/tip";       // 用户提醒主题
String photoTopic = "ZUCC-SYT/photo"; 
String reportTopic = "ZUCC-SYT/report";   // 系统报告主题
String reportDHT11 = "admintemp_hum/emqx";   // 系统报告主题
String userControlString;                       // 用户通过MQTT协议发来的设置信息
String userChatString;                          // 用户通过MQTT协议发来的聊天信息
String userTipString;  
String userPhotoString;  
uint8_t array_uint[2000];                     //图片存储数组
char photoStr[2000];
bool userChatFlag = false;                      // 告知系统是否有用户聊天信息需要显示
bool userTipFlag = false;                       // 告知系统是否有用户消息信息需要显示
bool userPhotoFlag = false;  
// TFT屏幕显示相关对象和变量
...
void setup() {
  ...
  wifiManager.autoConnect("IOT-SYT", "20010609");  

  #ifdef USE_MQTT
    // ---MQTT准备工作---
    // 设置MQTT服务器和端口号
    mqttClient.setServer(mqttServer, 11410);
    // 设置MQTT订阅回调函数
  ...
  //显示星期二字
  tft.drawBitmap(260,0,xing,20,20,BANNER_FG); // 显示”星“字
  tft.drawBitmap(280,0,qi1,20,20,BANNER_FG);  // 显示”期“字  
...
void loop(){
  if (timeStatus() != timeNotSet) {
    if (now() != prevDisplay) { //update the display only if time has changed
      ...
      //digitalClockDisplay();
        ...
        Serial.println(chatTopic); 
         Serial.println("tipTopic:");
        Serial.println(tipTopic);    
        Serial.println("photoTopic:");
        Serial.println(photoTopic);   
        Serial.println("reportTopic:");
        Serial.println(reportTopic);  
        Serial.println("reportDHT11:");
        Serial.println(reportDHT11);  
        Serial.println("");        
      #endif
  ...
  if (dht11UpdateFlag == true){
    dht11UpdateFlag = false;    
     pubReportDHT11();
  }
 // 更新bili信息 
...
void hex_2_uint8(const char* source, size_t length, uint8_t* target, size_t n) {//解析图片的字符串
    n = 0;
...
void displayBg(int bgId){
  ...
  if(userPhotoFlag == true){
    Serial.print("userPhotoFlag = ");Serial.println(userPhotoFlag);
    showPhotoMessage(userPhotoString);
    Serial.print("测试图片输出");
  }
  ...
  if(userTipFlag == true){
    Serial.print("userTipFlag = ");Serial.println(userTipFlag);
    showTipMessage(userTipString);
  }
...
void ntpInit(){
  ...
  setSyncInterval(timeCheckInterval);        // 每隔300秒钟对时一次
}
...
void tick(){
  ...
  //温湿度更新计时器
  dht11Counter++;
  if (dht11Counter > dht11UpdateInterval){
    dht11Counter = 0;
    dht11UpdateFlag = true;
  }
  // 达到背景变换时间
...
void connectMQTTserver(){
  ...
  // 连接MQTT服务器
  if (mqttClient.connect(clientId.c_str(),serveruser,serverpsw)) { 
    Serial.println("登录成功...");
...
void receiveCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if(String(topic) == controlTopic){
  ...
  else if(String(topic) == tipTopic){
    userTipString = "";
    for (int i = 0; i < length; i++) {
      userTipString += (char)payload[i];
    }
    Serial.print("userTipString = ");Serial.println(userTipString); 
  
    showTipMessage(userTipString);  // 屏幕上显示用户聊天消息
    pubReportMsg(userTipString);  // 通过系统报告主题发布用户聊天信息

  } 
  
   else if(String(topic) == photoTopic){
    //初始化字符串和数组
    userPhotoString = "";
    ...
    for (int i = 0; i < length; i++) {  
      userPhotoString += (char)payload[i];
//      array_uint[i]=(char)payload[i];
      photoStr[i]=(char)payload[i];
    }
    hex_2_uint8(photoStr, 400, array_uint, 200);//实现字符串解析，每两个字符串转成十六进制的格式
    Serial.print("userPhotoString = ");Serial.println(userPhotoString); 
    
//     Serial.print("array_uint = ");Serial.println(array_uint); 
//      for (int i = 0; i < length; i++) {  
//       Serial.print(array_uint[i]);Serial.print(array_uint[i]);
//    }
    showPhotoMessage(userPhotoString);  // 屏幕上显示用户图片消息
    pubReportMsg(userPhotoString);  // 通过系统报告主题发布用户图片信息
  } 
}
...
void pubReportDHT11(){
  ...
  String topicString = reportDHT11;
  char publishTopic[topicString.length() + 1];  
  strcpy(publishTopic, topicString.c_str());
...
serializeJson(doc ,output);
  ...
  // 实现ESP8266向主题发布信息
  if(mqttClient.publish(publishTopic, output.c_str())){
    Serial.println("Publish Report Topic:");Serial.println(publishTopic);
    Serial.println("Publish Report Message:");Serial.println(output.c_str());    
  } else {
    Serial.println("Report Message Publish Failed."); 
  }
}
...
void displayUserMessage(String userString){
    tft.setViewport(0, 190, BANNER_WIDTH, 30);
    tft.fillScreen(BANNER_BG);
    // 设置信息显示位置
...
//--- 显示用户消息信息相关函数开始 ---
void showTipMessage(String userTipString){

  if (userTipString == NULL){
    userTipFlag = false;  // 告知系统目前没有用户聊天信息
    displayBg(bgId);  
  } else {
    userTipFlag = true;  // 告知系统目前有用户聊天信息
    if(userTipString.length() > 50){
      userTipString = "[MESSAGE TOO LONG!]";
    } else {
      displayUserTip(userTipString);
    }
  }
}

void displayUserTip(String userString){
    tft.setViewport(0, 90, BANNER_WIDTH, 40);
//    tft.fillScreen(BANNER_BG);
    // 设置信息显示位置
    tft.setTextDatum(TC_DATUM);
    tft.setTextColor(TFT_RED, TFT_BLUE);  //设置文字颜色和背景色
    tft.setTextPadding(10); //设置文字padding，覆盖旧字  
...
//void showPhotoMessage(String userPhotoString){
void showPhotoMessage(String userPhotoString){
  if (userPhotoString == NULL){
    userPhotoFlag = false;  // 告知系统目前没有用户聊天信息
     Serial.print("告知系统目前没有图片信息");
    displayBg(bgId); 
  } else {
    userPhotoFlag = true;  // 告知系统目前有用户聊天信息
    if(userPhotoString.length() > 2000){
      userPhotoString = "[MESSAGE TOO LONG!]";
      Serial.print(">>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<");
    } else {
      Serial.print("<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>");
      Serial.print(userPhotoString);
      displayUserPhoto(userPhotoString);
    }
  }
}

//--- 显示用户消息信息相关函数结束 ---
void displayUserPhoto( String userPhotoString){
  // 设置信息显示位置

//  tft.fillRect(70, 50, 180, 140, BANNER_BG);
//  tft.drawBitmap(122,82,chnChar1,25,25,BANNER_FG);  // 显示图像
//  tft.drawBitmap(173,82,chnChar1,25,25,BANNER_FG);  // 显示图像
//  tft.drawBitmap(122,133,chnChar1,25,25,BANNER_FG);  // 显示图像
//  tft.drawBitmap(173,133,chnChar1,25,25,BANNER_FG);  // 显示图像
  
if(photoStr[0]=='1'){
  
  fex.drawJpeg("/bg" + String(4) + ".jpg", 0, BANNER_HEIGHT);
}
else if(photoStr[0]=='2'){
  
  fex.drawJpeg("/bg" + String(11) + ".jpg", 0, BANNER_HEIGHT); 
}
else if(photoStr[0]=='3'){
  
  fex.drawJpeg("/bg" + String(13) + ".jpg", 0, BANNER_HEIGHT); 
}
else if(photoStr[0]=='4'){
  
  fex.drawJpeg("/bg" + String(14) + ".jpg", 0, BANNER_HEIGHT); 
}
else if(photoStr[0]=='5'){
  
  fex.drawJpeg("/bg" + String(15) + ".jpg", 0, BANNER_HEIGHT); 
}
else if(photoStr[0]=='6'){
  
  fex.drawJpeg("/bg" + String(16) + ".jpg", 0, BANNER_HEIGHT); 
}
else if(photoStr[0]=='7'){
  
  fex.drawJpeg("/bg" + String(17) + ".jpg", 0, BANNER_HEIGHT); 
}
else if(photoStr[0]=='8'){
  
  fex.drawJpeg("/bg" + String(24) + ".jpg", 0, BANNER_HEIGHT); 
}
else if(photoStr[0]=='9'){
  
  fex.drawJpeg("/bg" + String(26) + ".jpg", 0, BANNER_HEIGHT); 
}
}
